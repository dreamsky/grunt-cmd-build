    ...
    exports.jsConcat = function(fileObj, options) {
        ...
        var rv = meta.dependencies.map(function(dep) {
            ...
            if (options.include === 'relative') {
                ...
                var fpath = path.join(options.paths, dep);
                logs.push('  ' + dep);
                if (!/\.js$/.test(fpath)) fpath += '.js';
                if (!grunt.file.exists(fpath)) {
                    if (!/\{\w+\}/.test(fpath)) {
                        grunt.log.warn('file ' + fpath + ' not found');
                        logs.push('  【Error】file ' + fpath + ' not found');
                    }
                    return '';
                }
        ...
        return {
                data:[data, rv].join(grunt.util.normalizelf(options.separator)),
                logs: logs,
